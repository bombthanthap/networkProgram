...
def acceptconnect():
    #w8 for client queue up to 5 request
    soc.listen(5)
    ...
    try:
        if(mode == 'pvp'):
            ...
            start_new_thread(ClientReceive2,(c,c2,"O"))  #client 1 
            start_new_thread(ClientReceive2,(c2,c,"X"))  #client 2
            reset()
        else:
            print ("== ::SERVER Established")
            c , add = soc.accept()
...
def openhof():
    ...
    lblScore = Label(MHFrameRight,font=('arial',20,'bold'), text=" Player Score",bg="White",width=15,borderwidth=2, relief="solid")
...
def send2(x,count,isWin,isDraw,who):
    ...
    try:
        ...
        reset()
        print("Error with sendall function")
        print('error fuc send2')
        tkinter.messagebox.showwarning("Warnning", "Plz connect again")
        print("Plz connect again")


# ---------- fuc to receive case 1 ---------- #
def ClientReceive(c):
    global PlayerO
    global turn
    global count
    ...
    while True:
        try:
            ...
            print("Error fuc ClientReceive")
            tkinter.messagebox.showwarning("Warnning", "Player has Disconnected")
            break
        try:
            PlayerX.set(scoreX)
            x = x-1
...
def ClientReceive2(c,c2,mark):
    ...
    while True:
        try:
            ...
            reset()
            print("Error with receive function")
            print("error fuc ClientReceive2")
            tkinter.messagebox.showwarning("Warnning", "Player has Disconnected")
            break
